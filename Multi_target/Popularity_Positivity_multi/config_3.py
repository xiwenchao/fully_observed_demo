...
class _Config():
    ...
    def init_basic(self):
        ...
        self.new_users_dict = dict()
        for row in self.test_list:
            user, item = row[1], row[2]
            if str(user) not in self.new_users_dict.keys():
                self.new_users_dict[str(user)] = list()
            self.new_users_dict[str(user)].append(int(item))

        avg_len = 0
        for user in self.new_users_dict:
            avg_len += len(self.new_users_dict[user])


        self.recommended_list = dict()
        for user in self.user_list:
            self.recommended_list[str(user)] = list()
    ...
    def random_sample(self, alpha):
        ...
        for user in self.new_users_dict:
            avg_len += len(self.new_users_dict[user])
        # print("avg_len = {}".format(avg_len / len(self.new_users_dict)))
