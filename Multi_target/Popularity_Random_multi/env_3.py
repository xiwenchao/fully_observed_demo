...
class user():
    ...
    def inform_facet(self, facet):
        ...
        ground_truth_feature = list()
        for item in cfg.new_users_dict[str(self.user_id)]:
            ground_truth_feature += cfg.item_dict[str(item)]['feature_index']
    ...
    def response(self, input_message):
        ...
        if input_message.message_type == cfg.MAKE_REC:
            ...
            success_items = list(set(cfg.new_users_dict[str(self.user_id)]) & set(rec_list))
            ...
            if len(success_items) == 0:
                ...
                data['ranking'] = 1
                data['total'] = 1
                print("success_items = {}".format(success_items))
                cfg.recommended_list[str(self.user_id)] += [success_items[0]]
                cfg.new_users_dict[str(self.user_id)].remove(success_items[0])
