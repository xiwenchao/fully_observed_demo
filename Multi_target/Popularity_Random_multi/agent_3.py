...
r_list = [-1, -0.5, -0.2, 0.5,
          1]  # quit by user ,fail to recommend, ask attribute not accepted, ask attribute accepted, accept recommendation
d = 64
...
class agent():
    ...
    def update_upon_feature_inform(self, input_message):
        ...
        if value is not None:
            self.recent_candidate_list = [k for k in self.recent_candidate_list if
                                          len(set(cfg.item_dict[str(k)]['feature_index']) & (set(value))) != 0]

            if type(value) != int:
                self.known_feature += [int(i) for i in value]  # Update known feature #
            else:
        ...
        if (value is not None and value[0] is not None) or self.turn_count == 1 or self.turn_count == 0:
            ...
            # print("len_candidate = {}".format(len(self.recent_candidate_list)))
    ...
    def prepare_rec_message(self):
        self.recent_candidate_list_ranked = [item for item in self.recent_candidate_list if
    ...
    def response(self, input_message):
        ...
        if cfg.play_by == 'policy':  # do policy gradient
            ...
            if cfg.eval == 1:  # means choose the action with highest probability
                action_max = 31
